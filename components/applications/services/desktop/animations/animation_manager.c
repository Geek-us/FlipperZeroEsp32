...
static void animation_manager_start_new_idle(AnimationManager* animation_manager) {
    ...
    uint32_t ticks = bubble_animation->duration * 1000;
    furi_timer_start(animation_manager->idle_animation_timer, ticks > 0 ? ticks : 1);
}
            ...
            if(restore_animation) {
                ...
                if(valid) {
                    ...
                    if(animation_manager->freezed_animation_time_left) {
                        uint32_t ticks = animation_manager->freezed_animation_time_left;
                        furi_timer_start(
                            animation_manager->idle_animation_timer,
                            ticks > 0 ? ticks : 1);
                    } else {
                        ...
                        uint32_t ticks = animation->duration * 1000;
                        furi_timer_start(
                            animation_manager->idle_animation_timer, ticks > 0 ? ticks : 1);
                    }
                }
            } else {
                FURI_LOG_E(
