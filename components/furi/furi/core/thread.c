...
#define THREAD_NOTIFY_INDEX 0 // ESP32-C5 only supports index 0

// Map Furi priority to FreeRTOS priority (0-24 range for ESP32-C5)
static inline UBaseType_t furi_thread_map_priority(FuriThreadPriority priority) {
    // FuriThreadPriorityIdle = -3 -> 1
    // FuriThreadPriorityLowest = -2 -> 5
    // FuriThreadPriorityLow = -1 -> 9
    // FuriThreadPriorityNormal = 0 -> 12
    // FuriThreadPriorityHigh = 1 -> 16
    // FuriThreadPriorityHighest = 2 -> 20
    // FuriThreadPriorityIsr = 24 -> 24
    const UBaseType_t base = 12; // Normal priority
    const UBaseType_t step = 4;
    
    if(priority >= FuriThreadPriorityIsr) {
        return (UBaseType_t)FuriThreadPriorityIsr;
    } else if(priority == FuriThreadPriorityIdle) {
        return 1;
    } else if(priority == 0) {
        return base;
    } else if(priority > 0) {
        UBaseType_t result = base + (priority * step);
        return (result < (UBaseType_t)FuriThreadPriorityIsr) ? result : ((UBaseType_t)FuriThreadPriorityIsr - 1);
    } else {
        // priority < 0 && priority > FuriThreadPriorityIdle
        int offset = priority + 3; // -2 -> 1, -1 -> 2
        return 1 + ((offset + 1) * step);
    }
}
...
void furi_thread_set_current_priority(FuriThreadPriority priority) {
    FuriThreadPriority prio = priority ? priority : FuriThreadPriorityNormal;
    UBaseType_t new_priority = furi_thread_map_priority(prio);
...
void furi_thread_start(FuriThread* thread) {
    ...
    FuriThreadPriority prio = thread->priority ? thread->priority : FuriThreadPriorityNormal;
    UBaseType_t priority = furi_thread_map_priority(prio);
    if(thread->is_service) {
