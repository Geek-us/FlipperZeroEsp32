...
FuriStatus furi_timer_start(FuriTimer* instance, uint32_t ticks) {
    ...
    /* Prevent 0 tick period which causes FreeRTOS timer assert */
    if(ticks == 0) {
        FURI_LOG_W(TAG, "furi_timer_start called with 0 ticks, coercing to 1");
        ticks = 1;
    }
...
FuriStatus furi_timer_restart(FuriTimer* instance, uint32_t ticks) {
    ...
    if(ticks == 0) {
        FURI_LOG_W(TAG, "furi_timer_restart called with 0 ticks, coercing to 1");
        ticks = 1;
    }
