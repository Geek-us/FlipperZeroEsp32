...
static uint8_t *i2c_buffer = NULL;          // I2C transmit buffer.
static size_t i2c_buffer_len = 0;           // I2C buffer length.
static u8g2_esp32_hal_t    u8g2_esp32_hal;  // HAL state data.

#undef ESP_ERROR_CHECK
#define ESP_ERROR_CHECK(x)   do { esp_err_t rc = (x); if (rc != ESP_OK) { ESP_LOGE("err", "esp_err_t = %d", rc); } } while(0);

// Compatibility for different ESP32 variants
#ifndef HSPI_HOST
...
uint8_t u8g2_esp32_i2c_byte_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	...
	switch(msg) {
		...
		case U8X8_MSG_BYTE_INIT: {
			...
			// Allocate buffer for I2C transactions (max size for u8g2)
			i2c_buffer = (uint8_t*)malloc(256);
			if (i2c_buffer == NULL) {
		...
		case U8X8_MSG_BYTE_SEND: {
			...
			if (i2c_buffer && (i2c_buffer_len + arg_int) <= 256) {
		...
		case U8X8_MSG_BYTE_START_TRANSFER: {
			...
			if (i2c_dev_handle == NULL && i2c_bus_handle != NULL) {
				...
				esp_err_t ret = i2c_master_bus_add_device(i2c_bus_handle, &dev_config, &i2c_dev_handle);
				if (ret != ESP_OK) {
		...
		case U8X8_MSG_BYTE_END_TRANSFER: {
			...
			if (i2c_dev_handle != NULL && i2c_buffer_len > 0) {
				esp_err_t ret = i2c_master_transmit(i2c_dev_handle, i2c_buffer, i2c_buffer_len, I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
				if (ret != ESP_OK) {
					ESP_LOGE(TAG, "I2C transaction failed: %d", ret);
				}
			}
